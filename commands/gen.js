                ...
                fs.readFile(filePath, function (error, data) {
                    ...
                    if (!error) {
                        ...
                        message.author.send(
                            ...
                            .setTitle('Here is your generated account :)')
                            .addField('Stock used :', `\`\`\`${args[0][0].toUpperCase()}${args[0].slice(1).toLowerCase()}\`\`\``, true)
                            .addField('Account', `\`\`\`${firstLine}\`\`\``, true)
                            .addField('Copy that :',`\`${firstLine}\``, false)
                            .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                            .setTimestamp()
                            
                        )
                        
                        .then(msg=>msg.delete({timeout:"60000"/*Time until delete in milliseconds*/}))
                        ...
                        if (position !== -1) {
                            ...
                            fs.writeFile(filePath, data, function (error) {
                                message.channel.send(
                                    ...
                                    .setDescription(`${message.author} *I sent you a ${args[0][0].toUpperCase()}${args[0].slice(1).toLowerCase()} item!*`)
                                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                    .setTimestamp()
                                );
                            ...
                            return message.channel.send(
                                ...
                                .setColor(config.color.red)
                                .setTitle('Generator error!')
                                .setDescription(`The \`${args[0]}\` stock is empty!`)
                                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                                .setTimestamp()
                            );
                        };
                        ...
                        return message.channel.send(
                            ...
                            .setColor(config.color.red)
                            .setTitle('Generator error!')
                            .setDescription(`Stock \`${args[0]}\` does not exist!`)
                            .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true, size: 64 }))
                            .setTimestamp()
                        );
                    };
